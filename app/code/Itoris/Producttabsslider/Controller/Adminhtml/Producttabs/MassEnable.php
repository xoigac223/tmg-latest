...
class MassEnable extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if($this->getRequest()->getParam('store')==NULL) {
            ...
            foreach($data as $tab){
                if($tab['store']==NULL){
                    $insert=$insert."({$tab['tab']},2,1,{$this->getRequest()->getParam('store')}), ";
                }else{
                    $tabGlobal[]=$tab['tab'];
                }
            }
        ...
        $this->messageManager->addSuccess(__('A total of %1 tab(s) have been enabled.', $count));

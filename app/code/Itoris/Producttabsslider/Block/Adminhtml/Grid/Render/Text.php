...
class Text extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\Text
{
    ...
    public function _getValue(\Magento\Framework\DataObject $row)
    {
        ...
        $prodGroup=$row->getData('group_prod_name');
        $store=$row->getData('group_store_name');
        if($store==NULL) {
            if (!empty($prodGroup)) {

                $prodGroup = explode(',', $prodGroup);
                if (count($prodGroup) > 1) {
                    if (in_array("''", $prodGroup)|| in_array('""', $prodGroup))
                        array_pop($prodGroup);
                }
                $prodGroup = implode(',', $prodGroup);
                $prodGroup = trim($prodGroup,",");
                    $row->setData('group_name', $prodGroup);

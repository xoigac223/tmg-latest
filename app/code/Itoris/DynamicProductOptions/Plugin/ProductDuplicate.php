...
class ProductDuplicate {
    ...
    public function afterCopy($subject, $result) {
        ...
        if (!$this->getDataHelper()->getSettings(true)->getEnabled()) return $result; //skip
        
        $this->_objectManager->get('Itoris\DynamicProductOptions\Model\Rewrite\Option')->duplicate((int)$request->getParam('id'), (int)$result->getId());

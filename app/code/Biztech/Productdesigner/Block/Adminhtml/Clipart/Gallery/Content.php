       ...
       \Magento\Framework\Data\Form\Element\Renderer\RendererInterface
{
      ...
      public function getImagesJson($id)
      {
          ...
          $images = $objectManager->create('Biztech\Productdesigner\Model\Mysql4\Clipartmedia\Collection')->addFieldToFilter("clipart_id",array("eq"=>$id));
          if (count($images)){
            foreach($images as $image){
              $demo = $objectManager->create('\Magento\Store\Model\StoreManagerInterface');
              $url = $demo->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) . 'productdesigner/clipart' . $image['image_path'];
              $image['url'] = $url;

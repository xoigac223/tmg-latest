 ...
 * Version: 1.4.3
 *
 * Modified by the Blackbird Team
 * 
 */
...
//		this.button.rotateccw	= '<div class="btn btn-default btn-rotate-ccw" title="' + (this.options.ccwTitle || 'Rotate counter clockwise') + '"><i class="glyphicon glyphicon-share icon-flipped"></i></div>';
	...
	$.html5imageupload.prototype = {
		_init: function() {
			...
			if (!_self.isEmpty(options.width) && _self.isEmpty(options.height) && $(element).innerHeight() <= 0) {
			...
			if (!_self.isEmpty(options.height) && _self.isEmpty(options.width) && $(element).innerWidth() <= 0) {
			...
			/*if (!_self.isEmpty(options.height) && !_self.isEmpty(options.width) && !_self.isEmpty($(element).innerHeight() && !_self.isEmpty($(element).innerWidth()))) {
			...
			if ($(element).innerWidth() > 0) {
				...
				width			= null
			} else if (!_self.isEmpty(options.width)) {
				width			= options.width;
			}
			...
			if ($(element).innerHeight() > 0) {
				...
				height			= null
			} else if (!_self.isEmpty(options.height)) {
			...
			if (!_self.isEmpty(options.image) && options.editstart == false) {
			...
			} else if (!_self.isEmpty(options.image)) {
		...
		handleFile: function(event, element) {
		   ...
		   // _self.reset();
		    if (options.removeurl != null && !_self.isEmpty($(element).data('name'))) {
				$.ajax({
					type: 'POST',
		...
		readImage: function(image, src, name, mimeType) {
    		...
    		img.onload 		= function(tmp) {
    			...
    			if (!_self.isEmpty(_self.imageGhost)) {
	            ...
	            imageObj.onload = function() {
	                ...
	                tmpObj.onload = function() {
	                	...
	                	if (options.save == false) {
	        				...
	        				$(input).after($('<input type="hidden" name="' + $(input).attr('name') + '_values" class="final" />').val(json));
...
/*		rotate: function(degrees) {
		...
		_ghost: function() {
			...
			if (options.ghost == true && !_self.isEmpty(ghost)) {

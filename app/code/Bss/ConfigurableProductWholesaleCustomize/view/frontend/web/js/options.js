...
window.DynamicProductOptions = {
            ...
            if (this.getAppearance() == 'popup_cart' || this.getAppearance() == 'popup_configure') {
                ...
                if (optionsWrapper) {
                    ...
                    optionsWrapper.select('*').each(function(elm){
                        if(jQuery(elm).attr('id') == "bss-ptd-table" )
                            return;
                        if (jQuery(elm).closest('#bss-ptd-table').length > 0)
                            return;
    ...
    prepareOptions : function() {
                ...
                if (origField && dynamicField) {
                    ...
                    if (this.options[i].type == 'radio' || this.options[i].type == 'checkbox') {
                        for (var j = 0; j < this.options[i].items.length; j++) {
                            if (typeof this.options[i].items[j] != 'undefined' && typeof this.options[i].items[j].elm != 'undefined') {
        ...
        this.getPriceBox().on('updatePrice', function(ev, prices){
            ...
            for(key in prices) {
                ...
                if (prices[key] && prices[key].basePrice) {
                    if(qtyElm) {
                        var qty = parseFloat(qtyElm.value);
                        ...
                        var tierPrice = obj.getTierPrice(qtyElm, prices[key].dpoPrice.amount, qty);
                        var optionPrice = jQuery("#bss-option-price").val();
                        if (tierPrice) {
                            optionPrice = parseFloat(optionPrice) + parseFloat(tierPrice);
    ...
    getTierPrice: function(qtyElm, price, qty) {
        var tierPrices = this.getTierPrices(qtyElm);
        if (tierPrices.length > 0) {
            for(var i=0; i<tierPrices.length; i++) if (qty >= parseInt(tierPrices[i].qty)) price = parseFloat(tierPrices[i].price);
        }
        /*custom */
        var elmPrice = 0 ;
        for( var QtyPrice in window.NewTierPrices){
            if(typeof window.NewTierPrices[qty] !== "undefined")
                { elmPrice =  window.NewTierPrices[qty].item_price;}
        }
        if(elmPrice != 0){
                price = parseFloat(elmPrice);
        }
        return price;
    },
    ...
    updateTierPrice : function() {
        var arrayOptions = [];
        var arrayProductionCharges = [];
        var arrayOptionsTierPrice = [];
        var obj = this;
        jQuery(".bss-option-price").val(0);
        var canReloadPrice = false;
        var canReloadPriceProductionCharges = false;
        var opConfig = obj.getOpConfig();
        var dynamicOptions = obj.getDynamicOptions();
        var pricing = dynamicOptions['config']['config']['absolute_pricing'];

        jQuery('#itoris_dynamicproductoptions select, #itoris_dynamicproductoptions textarea, #itoris_dynamicproductoptions input[type=text]').each(function(elm){
            if(jQuery(this).is(':visible') && !jQuery(this).is(':disabled')) {
                if(jQuery.mage.priceOptions && !arrayOptions[jQuery(this).attr('name')]) {
                    if(jQuery(this).attr('production_charges')) {
                        arrayProductionCharges[jQuery(this).attr('name')] = jQuery(this).val();
                        canReloadPriceProductionCharges = true;
                    }else{
                        arrayOptions[jQuery(this).attr('name')] = jQuery(this).val();
                        arrayOptionsTierPrice[jQuery(this).attr('name')] = jQuery(this).find(":selected").text();
                        canReloadPrice = true;
                    }
                }
            }
        ...
        // jQuery('#itoris_dynamicproductoptions input[type=radio]').each(function(elm){
        //     if(jQuery(this).is(':visible') && !jQuery(this).is(':disabled')) {
        //         if(jQuery.mage.priceOptions && !arrayOptions[jQuery(this).attr('name')]) {
        //             arrayOptions[jQuery(this).attr('name')] = jQuery(this).val();
        //             canReloadPrice = true;
        ...
        //     }      
        // });
        jQuery('#itoris_dynamicproductoptions .options-list').each(function(elm){
            if(jQuery(this).is(':visible') && !jQuery(this).is(':disabled')) {
                var valueArr = [];
                var key = "";
                jQuery(this).find(".checkbox").each(function(){
                    if(jQuery(this).is(':checked')) {
                        ...
                        arrayOptionsTierPrice[jQuery(this).attr('name')] = jQuery(this).next().find('span').text();
                        key = jQuery(this).attr('name');
                    }
                });
                if(valueArr.length > 0) {
                    if (jQuery(this).attr('production_charges')) {
                        arrayProductionCharges[key] = valueArr;
                        canReloadPriceProductionCharges = true;
                    } else {
                        arrayOptions[key] = valueArr;
                        canReloadPrice = true;
                    }
                }
            }
        });
        jQuery("#bss-total-price").val(0);
        if(canReloadPriceProductionCharges) {
            for (var key in arrayProductionCharges) {
                if(typeof key == "string" && typeof arrayProductionCharges[key] == "string") {
                    var optionId = key.replace("options","");
                    optionId = optionId.replace("[","");
                    optionId = optionId.replace("]","");
                    var prices = null;
                    try {
                        prices = opConfig['config'][optionId][arrayProductionCharges[key]];
                    ...
                    if(prices) {
                        if(qtyElm) {
                            var qty = parseFloat(qtyElm.value);
                            if (qtyElm.hasClassName('use_product_qty')) qty = jQuery(qtyElm) ? jQuery(qtyElm).val() : 1;
                            if (isNaN(qty) || qty < 1) qty = 1;

                            var tierPrice = obj.getTierPrice(qtyElm, prices.prices.finalPrice.amount, qty);
                            var optionPrice = 0;
                            if (tierPrice) {
                                optionPrice = parseFloat(optionPrice) + parseFloat(tierPrice);
                                jQuery("#bss-total-price").val(optionPrice);
                            }

                        }else {
                            var optionPrice = 0;
                            optionPrice = parseFloat(optionPrice) + parseFloat(prices.prices.finalPrice.amount);
                            // optionPrice = parseFloat(optionPrice);
                            jQuery("#bss-total-price").val(optionPrice);
                        }
                    }
                }else if(typeof key == "string" && typeof arrayProductionCharges[key] == "object") {
                    var optionId = key.replace("options","");
                    optionId = optionId.replace("[]","");
                    optionId = optionId.replace("[","");
                    optionId = optionId.replace("]","");
                    var totalOptionPrice = 0;
                    for (i = 0; i < arrayProductionCharges[key].length; i++) {
                        var prices = null;
                        try {
                            prices = opConfig['config'][optionId][arrayProductionCharges[key][i]];
                        } catch (e) {
                            console.log(e);
                        }
                    ...
                    jQuery("#bss-total-price").val(totalOptionPrice);
                }
            }
        }

        if(canReloadPrice) {
            for (var key in arrayOptions) {
                if(typeof key == "string" && typeof arrayOptions[key] == "string") {
                    var optionId = key.replace("options","");
                    optionId = optionId.replace("[","");
                    optionId = optionId.replace("]","");
                    ...
                    var prices = null;
                    var options = null;
                    var hasTierPriceNotLogin = [];
                    var hasTierPriceLogin = [];
                    try {
                        prices = opConfig['config'][optionId][arrayOptions[key]];
                    } catch (e) {/** prevent unexpected errors **/}

                    var qtyElm = obj.updateQtyElement(key);
                    for (var j = 0; j < obj.options.length; j++) {
                        if(typeof this.options[j].items != 'undefined' && obj.options[j].items[0].option_id == optionId)
                        {
                            options = obj.options[j];
                        }
                    }

                    if(options) {
                        for (var j = 0; j < options.items.length; j++) {
                            if (typeof options.items[j].title != 'undefined'
                                && (arrayOptionsTierPrice[key].indexOf(options.items[j].title + " +") >= 0 || arrayOptionsTierPrice[key] == options.items[j].title)) {
                                var res = options.items[j].customer_group.split(",");
                                // not login
                                if (res.indexOf("0") != -1 || options.items[j].customer_group == "") {
                                    if (options.items[j].tier_price && JSON.parse(options.items[j].tier_price).length > 0) {
                                        var tierPrices = JSON.parse(options.items[j].tier_price);
                                        for (var i = 0; i < tierPrices.length; ++i) {
                                            if (jQuery("#bss-option-price" + tierPrices[i].qty + "_2").val()) {
                                                var optionPrice = jQuery("#bss-option-price" + tierPrices[i].qty + "_2").val();
                                            } else {
                                                var optionPrice = 0;
                                            }
                                            optionPrice = parseFloat(optionPrice) + parseFloat(tierPrices[i].price);
                                            hasTierPriceLogin['#bss-option-price_2'] = optionPrice;
                                            jQuery("#bss-option-price" + tierPrices[i].qty + "_2").val(optionPrice);
                                        }
                                    } else if (options.items[j].price) {
                                        hasTierPriceLogin['#bss-option-price_no_tier_2'] = true;
                                        hasTierPriceLogin['#bss-option-price_2'] = options.items[j].price;
                                    }
                                }

                                // general
                                if (res.indexOf("1") != -1 || options.items[j].customer_group == "") {
                                    if (options.items[j].tier_price && JSON.parse(options.items[j].tier_price).length > 0) {
                                        var tierPrices = JSON.parse(options.items[j].tier_price);
                                        for (var i = 0; i < tierPrices.length; ++i) {
                                            if (jQuery("#bss-option-price" + tierPrices[i].qty + "_1").val()) {
                                                var optionPrice = jQuery("#bss-option-price" + tierPrices[i].qty + "_1").val();
                                            } else {
                                                var optionPrice = 0;
                                            }
                                            optionPrice = parseFloat(optionPrice) + parseFloat(tierPrices[i].price);
                                            hasTierPriceLogin['#bss-option-price_1'] = optionPrice;
                                            jQuery("#bss-option-price" + tierPrices[i].qty + "_1").val(optionPrice);
                                        }
                                    } else if (options.items[j].price) {
                                        hasTierPriceLogin['#bss-option-price_no_tier_1'] = true;
                                        hasTierPriceLogin['#bss-option-price_1'] = options.items[j].price;
                                    }
                                }
                            }
                        }
                    ...
                    if(prices) {
                        if(qtyElm) {
                            ...
                            if (tierPrice) {
                                optionPrice = parseFloat(optionPrice) + parseFloat(tierPrice);
                                jQuery("#bss-option-price").val(optionPrice);
                            }
                            jQuery('.bss-table-row').first().find('.bss-tierprice').each(function(){
                                var qty = 1;
                                if (qtyElm.hasClassName('use_product_qty')) qty = jQuery(this) ? jQuery(this).attr('attributeqty') : 1;
                                if (isNaN(qty) || qty < 1) qty = 1;

                                var tierPrice = obj.getTierPrice(qtyElm, prices.prices.finalPrice.amount, qty);
                                var optionPriceTier = jQuery("#bss-option-price"+qty).val();
                                if (tierPrice) {
                                    optionPriceTier = parseFloat(optionPriceTier) + parseFloat(tierPrice);
                                    jQuery("#bss-option-price"+qty).val(optionPriceTier);
                                    if(hasTierPriceLogin['#bss-option-price_no_tier_2'] == true) {
                                        jQuery("#bss-option-price" + qty + "_2").val(hasTierPriceLogin['#bss-option-price_2']);
                                    }

                                    if(hasTierPriceLogin['#bss-option-price_no_tier_1'] == true) {
                                        jQuery("#bss-option-price" + qty + "_1").val(hasTierPriceLogin['#bss-option-price_1']);
                                    }
                                }
                                if (pricing == 1) {
                                    jQuery(".bss-table-row").find('.bss-tierprice').find('p[data-amount]').attr('data-amount', 0);
                                    jQuery("#bss-check-type-pricing").val(pricing);
                                }
                            })
                        }else {
                            var optionPrice = jQuery("#bss-option-price").val();
                            ...
                            jQuery('.bss-table-row').first().find('.bss-tierprice').each(function(){
                                var qty = 1;
                                qty = jQuery(this) ? jQuery(this).attr('attributeqty') : 1;
                                if (isNaN(qty) || qty < 1) qty = 1;

                                var optionPriceTier = jQuery("#bss-option-price"+qty).val();
                                optionPriceTier = parseFloat(optionPriceTier) + parseFloat(prices.prices.finalPrice.amount);
                                jQuery("#bss-option-price"+qty).val(optionPriceTier);
                                if(hasTierPriceLogin['#bss-option-price_no_tier_2'] == true) {
                                    jQuery("#bss-option-price" + qty + "_2").val(hasTierPriceLogin['#bss-option-price_2']);
                                }
        ...
        // custom order sample
        var order_sample = false;
        jQuery('#itoris_dynamicproductoptions select').each(function(elm){
            if(jQuery(this).is(':visible') && !jQuery(this).is(':disabled')) {
                jQuery(this).find('option').each(function() {
                    if(jQuery(this).is(':selected') && jQuery(this).hasClass('order-sample')){
                        jQuery('#bss-ptd-table tbody td').each(function() {
                            if (jQuery(this).hasClass('bss-tierprice')) {
                                jQuery(this).hide();
                                var text_ = jQuery(this).attr('attributeqty') + '+';
                                var index = jQuery(this).parent().children().index(jQuery(this));
                                jQuery(this).parents('#bss-ptd-table').find('thead tr th').each(function() {
                                    if (jQuery(this).text() == text_) {
                                        jQuery(this).hide();
                                    }
                                })
                            }
                        });
                        if (jQuery('#bss-ptd-table .addprice-cs').length) {
                            jQuery('#bss-ptd-table .addprice-cs').remove();
                            jQuery('p.addprice-cs').remove();
                        }

                        var parent = jQuery('#bss-ptd-table thead tr th.login_price');
                        if(!parent.length) {
                            parent = jQuery('#bss-ptd-table thead tr th').first();
                        }
                        jQuery('<th class="addprice-cs">Price</th>').insertAfter(parent)
                        jQuery('#bss-ptd-table tbody tr').each(function() {
                            var parent = jQuery(this).find('td.login_price');
                            if(!parent.length) {
                                parent = jQuery(this).find('td').first();;
                            }
                            var price__ = jQuery(this).find('td.bss-tierprice').first().html();
                            jQuery('<td class="addprice-cs">'+ price__ +'</td>').insertAfter(parent);
                        })
                        jQuery('<p class="addprice-cs"> <span>'+ jQuery('#text_qty_order_sample').val() +'</span></p>').insertAfter(jQuery('#bss-ptd-table'));
        ...
        if (!order_sample) {
            jQuery('.addprice-cs').remove();
            jQuery('#bss-ptd-table td,#bss-ptd-table th').show();
        }
        // end
        jQuery('.bss-qty').change();
        jQuery('#qty').change();
    },
    ...
    getDynamicOptions : function() {
        if (this.config.is_grouped) {
            return eval('dynamicOptions' + this.config.product_id);
        }
        return {config: window['dynamicOptions' + this.config.product_id]};
    ...
    negativePriceCheck: function() {
        ...
        if (jQuery.mage.priceOptions) {
            ...
            var refreshInterval =  setInterval(function () {
                if (jQuery('#bss-option-price').val() == '0') {
                    obj.updateTierPrice();
                } else {

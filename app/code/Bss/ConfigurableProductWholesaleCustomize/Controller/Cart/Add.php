...
namespace Bss\ConfigurableProductWholesaleCustomize\Controller\Cart;
...
class Add extends CheckoutAdd
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($this->isOrderSample()) {
                if ($this->maxOrderSample()) {
                    $this->messageManager->addErrorMessage(
                        __('Can\'t add this item to your shopping cart. The maximum sample quantity can be ordered for %1 is %2',$product->getName(),$product->getQtyOrderSample())
                    );
                    return $this->goBack();
                }
            }
    ...
    protected function maxOrderSample()
    {   
        $total_cart = $total_qty = 0;
        $cartItems = $this->cart->getQuote()->getAllVisibleItems();
        $productId = $this->getRequest()->getParam('product');
        $product = $this->_initProduct();
        $qtys = $this->getRequest()->getParam('bss-qty');
        if (!empty($qtys)) {
            foreach ($qtys as $qty) {
                if ($qty <= 0) {
                    continue;
        ...
        foreach ($cartItems as $item){
            if ($item->getProductId() == $productId && $this->isOrderSampleCart($product,$item)) {
                $total_cart += (int)$item->getQty();
            }
        }

        $is_max_qty =  ($total_cart + $total_qty) - (int)$product->getQtyOrderSample();
        if ($is_max_qty > 0) {
           return true;
        }
        return false;
    ...
    protected function isOrderSampleCart($product, $item)
    {
        ...
        foreach ($options as $option) {
            ...
            if ($option->getType() === 'drop_down') {
                ...
                foreach ($values as $value) {
                    if ($value->getId() == $customOptionItem[$option->getId()] && $value->getCssClass() == 'order-sample') {
                        return true;
                    }
                }
            }
    ...
    protected function isOrderSample()
    {
        $product = $this->_initProduct();
        $options = $product->getOptions();
        $_options = $this->getRequest()->getParam('options');
        if ($_options) {
            foreach ($options as $option) {
                if (!isset($_options[$option->getId()])) {
                    continue;
                }
                if ($option->getType() === 'drop_down') {
                    $values = \Magento\Framework\App\ObjectManager::getInstance()->create('Magento\Catalog\Model\Product\Option\Value')->getValuesCollection($option);
                    foreach ($values as $value) {
                        if ($value->getCssClass() == 'order-sample' && $_options[$option->getId()] == $value->getOptionTypeId()
                        ) {
                            return true;
                        }
                    }

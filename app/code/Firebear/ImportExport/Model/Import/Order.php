...
class Order extends AbstractEntity implements ImportAdapterInterface
{
    ...
    protected function createAddresses($orderIds)
    {
        ...
        $select = $this->getConnection()
            ->select()
            ->from(['so' => $orderTable], ['entity_id', 'customer_id', 'customer_email', 'customer_firstname', 'customer_lastname'])
            ->joinLeft(
                ['soa' => $addressTable],
    ...
    protected function createPayments($orderIds)
    {
        ...
        $select = $this->getConnection()
            ->select()
            ->from(['so' => $orderTable], ['entity_id', 'total_due', 'base_total_due', 'shipping_amount', 'base_shipping_amount'])
            ->joinLeft(
                ['sop' => $paymentTable],
        ...
        foreach ($ordersWoPayments as $row) {
            ...
            $paymentData[] = [
                ...
                'method' => \Magento\OfflinePayments\Model\Checkmo::PAYMENT_METHOD_CHECKMO_CODE
            ];
        }

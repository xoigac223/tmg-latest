...
class RevSliderAdmin extends RevSliderFunctions {
	...
	protected static function add_media_upload_includes(){
		...
		$this->_frameworkHelper->wp_enqueue_script('thickbox');
		$this->_frameworkHelper->wp_enqueue_script('media-upload');
		$this->_frameworkHelper->wp_enqueue_style('thickbox');
	}
	...
	public function add_slider_meta_box($post_types = null){
		try {
			add_meta_box('mymetabox_revslider_0', 'Slider Revolution Options', array('\Nwdthemes\Revslider\Model\Revslider\Admin\RevSliderAdmin', 'add_meta_box_content'), $post_types, 'normal', 'default');
		} catch (\Exception $e){}
	}
	...
	public static function custom_post_fields_output(){
		...
		<ul class="revslider_settings">
			<li id="slide_template_row">
				<div title="" class="setting_text" id="slide_template_text"><?php echo __('Choose Slide Template', 'revslider');?></div>
	...
	function modify_admin_body_class($classes){
		if($this->get_val(Data::$_GET, 'page') == 'revslider' && $this->get_val(Data::$_GET, 'view') == 'slide'){
		...
		try{
			...
			switch($action){
					...
					if(intval($slider_id) > 0 && empty($slider_data)){
						...
						foreach($slider_data as $sk => $sd){
							...
							if(strpos($sk, 'static_') !== false){
								...
								$sortOrder = $this->get_val($sd, 'sortOrder');
								$_slides[$sortOrder] = array(
									'id'		=> $sk,
									'slider_id'	=> $slider_id,
									'slide_order' => $sortOrder,
									'params'	=> stripslashes($this->get_val($sd, 'params')),
									'layers'	=> stripslashes($this->get_val($sd, 'layers')),
				...
				case 'check_system':
					...
					$fun = new RevSliderFunctions($this->_frameworkHelper);
	...
	public function get_javascript_multilanguage(){
		$lang = array(
			...
			'embedingLine5' => __('From the	<b>XML layout</b> use:', 'revslider'),
			'embedingLine6' => __('From the	<b>code</b> use:', 'revslider'),
	...
	public function activate_plugin($code){
		...
		if($this->_frameworkHelper->is_wp_error($version_info)){
			return false;
		}
	...
	public function deactivate_plugin(){
		...
		if($this->_frameworkHelper->is_wp_error($vi)){
			return false;
		}

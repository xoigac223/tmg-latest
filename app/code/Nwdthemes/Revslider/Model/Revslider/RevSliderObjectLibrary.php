...
class RevSliderObjectLibrary extends RevSliderFunctions {
	...
	public function __construct(
		...
		$this->font_icon_paths = array(Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/css/font-awesome.css', Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css');
	...
	public function _get_object_thumb($object_handle, $type, $download = false){
		...
		if($_download && $this->_frameworkHelper->getFilesystemHelper()->wp_mkdir_p($upload_dir['basedir'].$path) && $download === true){
			$curl = $this->_curl;
			$file = $upload_dir['basedir'] . $path . $object_handle;

...
class RevSliderOutput extends RevSliderFunctions {
	...
	public function add_slider_base(){
		try{
			 ...
			 **/
			if(version_compare($this->get_val($this->slider, array('settings', 'version')), $this->_frameworkHelper->get_option('revslider_update_revision', '6.0.0'), '<')){
	...
	public function add_layer($row_group_uid = false, $special_type = false){
		...
		$this->_frameworkHelper->do_action('revslider_add_layer_attributes', $layer, $this->slide, $this->slider);
...
rs-module .material-icons {
  ...
  -moz-osx-font-smoothing: grayscale;
	...
	public function get_html_layer_action(&$html_simple_link){
		...
		if(!empty($actions)){
			foreach($actions as $num => $action){
				...
				// Filter the Actions
                $events = $this->_frameworkHelper->apply_filters('rs_action_output_layer_action', $events, $action, $all_actions, $num, $this->slide);
			...
			case 'image':
				...
				if($img_size !== 'full'){
					if($cur_img_id !== false && !empty($cur_img_id)){
						$urlImage = $this->_frameworkHelper->wp_get_attachment_image_src($cur_img_id, $img_size);
						$urlImage = $urlImage[0];
					}
				}
	...
	public function get_html_slide_title(){
		...
		return ($title !== '') ? ' data-title="'.stripslashes($this->_frameworkHelper->esc_attr($title)).'"' : '';
	}
	...
	public function remove_slide_if_mobile($slides){
		...
		$mobile = (strstr($usragent,'Android') || strstr($usragent,'webOS') || strstr($usragent,'iPhone') ||strstr($usragent,'iPod') || strstr($usragent,'iPad') || strstr($usragent,'Windows Phone') || $this->_frameworkHelper->wp_is_mobile()) ? true : false;
	...
	private function add_javascript_to_footer(){
		...
		$slver = $this->_frameworkHelper->apply_filters('revslider_remove_version', Framework::RS_REVISION); //allows to remove slider version at the JavaScript and CSS inclusions

		$ret = RS_T3.'<script type="text/javascript" src="'.Framework::$RS_PLUGIN_URL_CLEAN.'public/assets/js/revolution.tools.min.js?rev='.$slver.'"></script>'."\n";

		if(!file_exists(Framework::$RS_PLUGIN_PATH.'public/assets/js/rs6.min.js')){
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.main.js?rev='.$slver.'"></script>'."\n";
			//if on, load all libraries instead of dynamically loading them
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.actions.js?rev='.$slver.'"></script>'."\n";
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.carousel.js?rev='.$slver.'"></script>'."\n";
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.layeranimation.js?rev='.$slver.'"></script>'."\n";
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.navigation.js?rev='.$slver.'"></script>'."\n";
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.panzoom.js?rev='.$slver.'"></script>'."\n";
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.parallax.js?rev='.$slver.'"></script>'."\n";
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.slideanims.js?rev='.$slver.'"></script>'."\n";
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/dev/rs6.video.js?rev='.$slver.'"></script>'."\n";
		}else{
			$ret .= RS_T3.'<script type="text/javascript" src="'. Framework::$RS_PLUGIN_URL_CLEAN . 'public/assets/js/rs6.min.js?rev='.$slver.'"></script>'."\n";
		}

		/**
		 * removed in 6.0
		 * $ret .= ($this->get_val($settings, 'enable_logs', 'off') == 'on') ? RS_T3.'<script type="text/javascript" src="'.Framework::$RS_PLUGIN_URL_CLEAN.'public/assets/js/jquery.themepunch.enablelog.js?rev='.$slver.'"></script>'."\n" : '';
		 **/
	...
	public function add_inline_double_jquery_error(){
		if(Framework::$rs_double_jquery_script === false){
		...
		Framework::$rs_double_jquery_script = true;
	}
	...
	private function add_js(){
		...
		if($me === true){ //for html markup export
			...
			if($this->_frameworkHelper->has_action('wp_footer', array($this, 'add_inline_double_jquery_error')) === false){
				$this->_frameworkHelper->add_action('wp_footer', array($this, 'add_inline_double_jquery_error'));
			}
		}
	}

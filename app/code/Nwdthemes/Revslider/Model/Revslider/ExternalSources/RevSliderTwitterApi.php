...
class RevSliderTwitterApi extends RevSliderFunctions {
	...
	*/
	public function __construct(
		...
		if(!$this->bearer_token = $this->_frameworkHelper->get_option('twitter_bearer_token'))
	...
	*/
	private function get_bearer_token(){
		...
		$response = $this->_frameworkHelper->wp_remote_post('https://api.twitter.com/oauth2/token', $args);

		if($this->_frameworkHelper->is_wp_error($response) || 200 != $response['response']['code'])
		...
		$this->_frameworkHelper->update_option('twitter_bearer_token', $this->get_val($result, 'access_token'));
	...
	*/
	public function query($query, $query_args = array(), $stop = false){
		...
		if($this->get_val($this->query_args, 'cache', 0) > 0 && false !== ($data = $this->_frameworkHelper->get_transient($transient_name)))
		...
		$response = $this->_frameworkHelper->wp_remote_get('https://api.twitter.com/1.1/'. $this->get_val($this->query_args, 'type') . '.json?' . $query, $args);
		if($this->_frameworkHelper->is_wp_error($response) || 200 != $response['response']['code']){
			if(!$stop){
				$this->bearer_token = $this->get_bearer_token();
		...
		$this->_frameworkHelper->set_transient($transient_name, $response['body'], $this->query_args['cache']);
	...
	*/
	private function bail($error_text, $error_object = ''){
		...
		if($this->_frameworkHelper->is_wp_error($error_object)){

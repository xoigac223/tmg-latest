...
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Exception\InstagramAuthException;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Exception\InstagramException;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Exception\InstagramNotFoundException;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Model\Account;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Model\Comment;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Model\Like;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Model\Location;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Model\Media;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Model\Story;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Model\Tag;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Model\UserStories;
use InvalidArgumentException;
use \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Unirest\Request;
...
class Instagram
{
    ...
    public function getAccount($username)
    {
        ...
        $response = \Nwdthemes\Revslider\Model\Revslider\ExternalSources\InstagramScraper\Unirest\Request::get(Endpoints::getAccountPageLink($username), $this->generateHeaders($this->userSession));
        if (static::HTTP_NOT_FOUND === $response->code) {
            throw new InstagramNotFoundException('Account with given username does not exist.');
        }
        if (static::HTTP_OK !== $response->code) {
    ...
    public function getMediasByUserId($id, $count = 12, $maxId = '')
    {
        ...
        while ($index < $count && $isMoreAvailable) {
            ...
            if (static::HTTP_OK !== $response->code) {
            	return null;
                //throw new InstagramException('Response code is ' . $response->code . '. Body: ' . static::getErrorBody($response->body) . ' Something went wrong. Please report issue.2');
            }
            $arr = json_decode($response->raw_body, true, 512, JSON_BIGINT_AS_STRING);
            if (!is_array($arr)) {
            	return null;
               // throw new InstagramException('Response code is ' . $response->code . '. Body: ' . static::getErrorBody($response->body) . ' Something went wrong. Please report issue.3');
            }
            ...
            foreach ($nodes as $mediaArray) {
                ...
                $medias['edge_owner_to_timeline_media']['edges'][] = $mediaArray['node'];
                $index++;
            }

...
class RevSliderSlider extends RevSliderFunctions {
	...
	public function init_by_id($sid){
		...
		if(empty($slider_data) && !$this->_frameworkHelper->is_admin()){
	...
	public function init_by_alias($alias){
		...
		if(empty($slider_data) && !$this->_frameworkHelper->is_admin()){
			throw new \Exception('Slider with alias '.$this->_frameworkHelper->sanitize_text_field(esc_attr($alias)).' not found.');
		}
	...
	private function duplicate_slider($title = false, $prefix = false){
		...
		$c_slider	= new RevSliderSlider($this->_frameworkHelper);

...
(function() {
	...
	RVS.F.drawEditorNavigation = function() {
		...
		switch (RVS.nav.currentSkinType) {
			...
			case "bullets":
				...
				RVS.nav.c.bullets.find('.tp-bullet').each(function(i) {
					...
					if (RVS.nav.currentAlignMode==="vertical") {
						...
						punchgs.TweenLite.set(RVS.nav.c.bullets,{height:(((am-1)*h) + b.outerHeight()),width:b.outerWidth()});
					}
					else {
						b.css({left:((i)*w)+"px", top:"0px"});
						punchgs.TweenLite.set(RVS.nav.c.bullets,{width:(((am-1)*w) + b.outerWidth()),height:b.outerHeight()});
					}
				});
			break;
			case "tabs":
			case "thumbs":
				...
				punchgs.TweenLite.set(RVS.nav.c[all].self,{padding:RVS.nav.currentTestPadding});
				...
				RVS.nav.c[all].inner.find('.'+RVS.nav.c[all].single).each(function(i) {
					if (RVS.nav.currentAlignMode==="vertical")
						punchgs.TweenLite.set(this,{top:((i)*h)+"px",left:"0px",width:RVS.nav.currentSkin.dim.width+"px",height:RVS.nav.currentSkin.dim.height+"px"});
					else
						punchgs.TweenLite.set(this,{left:((i)*w)+"px", top:"0px",width:RVS.nav.currentSkin.dim.width+"px",height:RVS.nav.currentSkin.dim.height+"px"});
				});
				...
				var maxw = RVS.nav.currentAlignMode==="horizontal" ? (RVS.nav.currentSkin.dim.width * am) + (RVS.nav.currentTestSpace*(am-1)) : RVS.nav.currentSkin.dim.width,
					...
					maskcssobj = {top:"auto", left:"auto", bottom:"auto", maginTop:"0px", marginBottom:"0px",right:"auto", y:"0%", x:"0px",width:maxw+"px",height:maxh+"px",overflow:"hidden",position:"relative",overwrite:"auto",marginLeft:"auto",marginRight:"auto"};
					/* innecssobj = {y:"0px",x:"0px",position:"relative"}; */
				punchgs.TweenLite.set(RVS.nav.c[all].self,wrapcssobj);
				punchgs.TweenLite.set(RVS.nav.c[all].mask,maskcssobj);
			break;
		}
	};
	...
	RVS.F.savechangesonnavigation = function() {
		...
		if (RVS.nav.currentHandle!==undefined) {
			...
			if (lastselected!==undefined)
				jQuery('#rs_ne_nav_skin_'+lastselected).trigger('click');
		}
		RVS.nav.toDelete = [];
		RVS.nav.toSave = false;
	...
	*/
	RVS.F.bgUpdate = function(_, val) {
		punchgs.TweenLite.set(_thumbstabs[_].self,{background:val});
	};
	...
	*/
	...
	var cpCallbacks = {
		...
		init: function(inputWrap, inputElement, cssColor, widgetSettings) {
			var ghost = jQuery('<input type="text" class="layerinput">').appendTo(inputWrap);// hide the real input and replace with a fake input for visual purposes
			inputElement.data('ghost', ghost).hide();// if the original input is hidden in any way, RevSlider admin won't include it in the input fields to update for some reason
		},

		onRefresh: function(inputElement, cssColor) {
			inputElement.data('ghost').val(cssColor);// update the ghost input field
		},

		onEdit: function(currentInput, cssColor) {
			currentInput.data('ghost').val(cssColor);// update the ghost input field
		...
		for (var fields in placeholders) {
			...
			switch (field.type) {
				...
				case "color-rgba":
					...
					jQuery('#'+prefix+fields).rsColorPicker({

						init: cpCallbacks.init,
						onRefresh: cpCallbacks.onRefresh,
						onEdit: cpCallbacks.onEdit,
						change: cpCallbacks.change,
						cancel: cpCallbacks.cancel
	...
	*/
	RVS.F.drawNavigation = function(obj) {
		...
		if (navobj!==undefined && navobj.markup!==undefined) {
			var cname = RVS.F.sanitize_input(navobj.name.toLowerCase()),
			...
			if (obj.type==="tabs" || obj.type==="thumbs") {
				if (obj.presetChange) {
					...
					punchgs.TweenLite.set(_thumbstabs[obj.type].self,{background:window.RSColor.get(_.wrapperColor)});
				}
	...
	*/
	function putNavObjInPos(el,_) {
		...
		punchgs.TweenLite.set(el,animobj);
	}
	 ...
	 RVS.F.sliderNavPositionUpdate = function(obj) {
		...
		if (obj.type==="tabs" || obj.type==="thumbs") {
			...
			punchgs.TweenLite.set(_thumbstabs[obj.type].self,{padding:_.padding});
			_.padding = parseInt(_.padding,0);

			// SET BULLET SPACES AND POSITION
			_thumbstabs[obj.type].inner.find('.'+_thumbstabs[obj.type].single).each(function(i) {
				if (_.direction==="vertical")
					punchgs.TweenLite.set(this,{top:((i)*h)+"px",left:"0px",width:_.width+"px",height:_.height+"px"});
				else
					punchgs.TweenLite.set(this,{left:((i)*w)+"px", top:"0px",width:_.width+"px",height:_.height+"px"});
			});
			...
			punchgs.TweenLite.set(_thumbstabs[obj.type].self,wrapcssobj);
			punchgs.TweenLite.set(_thumbstabs[obj.type].mask,maskcssobj);
